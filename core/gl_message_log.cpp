...
static void APIENTRY GLDebugCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const char* message, const void* userParam) {
	...
	if (type == GL_DEBUG_TYPE_ERROR || type == GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR) {
		// log all catastrophic problems to the console 
		printLog("[OpenGL] '%s' of severity '%s' from '%s: %s", DebugTypeString(type), DebugSeverityString(severity), DebugSourceString(source), message);
//		assert(false);
		setDevMenuOpen(GL_MESSAGE_LOG_WINDOW_NAME, true);
	}
...
void glMessageLogUI(bool* is_open) {
	...
	if (gui::Begin(GL_MESSAGE_LOG_WINDOW_NAME, is_open)) {
		static int which = 0;
			...
			if (which == 0 || messages[i].type == which_enums[which]) {
				...
				if (hover) {
					...
					if (messages[i].first_timestamp != messages[i].last_timestamp) {
						gui::Text("First Timestamp: %lld.%0*lld", messages[i].first_timestamp / osTimerFrequency(), freq_digs, messages[i].first_timestamp % osTimerFrequency());
						gui::Text(" Last Timestamp: %lld.%0*lld", messages[i].last_timestamp / osTimerFrequency(), freq_digs, messages[i].last_timestamp % osTimerFrequency());
					} else {
						gui::Text("Timestamp: %lld.%0*lld", messages[i].first_timestamp / osTimerFrequency(), freq_digs, messages[i].first_timestamp % osTimerFrequency());
					}
