...
struct TimerHash {
			...
			*added = true;
			if (last_found->hs.str == 0) { // blank entry
				...
				if (block_next_idx < TIMER_COUNT_MAX) { // make a new entry
					...
					printLog("[Timer] Block allocator full!\n");
					ASSERT(false);
					*added = false;
					return 0;
				}
	...
	void dev_stats() {
		...
		printLog("%d table entries (%3.2f), chained %d/%d (%3.2f), %d block entries (%3.2f), longest chain %d, avg chain %3.2f, chain count %d\n",
...
void TimestampLog::report(s64 t_frame_start, s64 t_frame_start_next, const s64* timestamps, int timestamps_count, TimestampReport* report) {
	...
	for (int i = 0; i < entries.count; ++i) {
		...
		if (e.type == TIMESTAMP_MARKER) {
			...
			Range& r = ranges.push();
			r.depth = (int)range_stack.count;
			r.t_start = timestamps[i];
			range_stack.push((int)ranges.count-1);
			HashedString hs = e.label;
			hs.hash = WangHash(hash_stack.back().hash ^ hs.hash);
			hash_stack.push(hs);
			r.label = hs;
			bool added;
			...
			Range& r = ranges[range_stack.back()];
			r.t_stop = timestamps[i];
			range_stack.pop();
			if (TimerHashEntry* s = sum_table.get(hash_stack.back())) {
				s->t_sum += r.t_stop - r.t_start;
				s->t_sum_count += 1;
				hash_stack.pop();
			}
...
void timestampReportGui(const char* id, const TimestampReport* report, bool* show_timeline, float ms_per_frame, double msec_per_count) {
	...
	*/
	float frame_ms = float((ranges[0].t_stop - ranges[0].t_start) * msec_per_count);
	...
	const int label_count = (int)range_sums.count;
	...
	if (gui::Begin(id, 0, ImGuiWindowFlags_AlwaysAutoResize)) {
		...
		for (int i = 0; i < range_sums.count; ++i) {
			float ms = float(range_sums[i].t_sum / float(range_sums[i].t_sum_count) * msec_per_count);
		...
		if (*show_timeline) {
			for (int i = 0; i < ranges.count; ++i) {
				float ms = float((ranges[i].t_stop - ranges[i].t_start) * msec_per_count);
				...
				*/
				...
				vec2 s = vec2(bars_x_start + float(double(ranges[i].t_start - time_of_frame_start) * msec_per_count * pixels_from_ms), 0.0f);
				vec2 e = vec2(bars_x_start + float(double(ranges[i].t_stop - time_of_frame_start) * msec_per_count * pixels_from_ms), gui::GetTextLineHeightWithSpacing());
				...
				if (ms > 1.0) {
					char digits[32];
					int len = sprintf_s(digits, "%.1f", ms);
					dl->AddText(cp + vec2((s.x + e.x)*0.5f - gui::GetFont()->FallbackAdvanceX * len * 0.5f, s.y), 0xffffffff, digits);
...
void CpuTimestampLog::newFrame() {
	s64 t_frame_start_next = getTime();
	log.report(t_frame_start, t_frame_start_next, timestamps.ptr, (int)timestamps.count, &report);
	timestamps.clear();
	log.clear();
...
void ctimer_gui() {
	float ms_per_frame = 1000.0f / 60.0f;//evk.win.RefreshRate;
	timestampReportGui("CPU TIMERS", &ctimer.report, &ctimer.show_timeline, ms_per_frame, 1000.0 / double(osTimerFrequency())); // ticks per second -> milliseconds per tick
}
...
void GpuTimestampLog::newFrame(VkCommandBuffer frame_cb) {
	...
	/* Compose a report */
	{
		...
		if (frames[r].timestamps.count && frames[n].timestamps.count) {
			...
			frames[r].log.report(frames[r].timestamps[0], frames[n].timestamps[0], frames[r].timestamps.ptr + 1, (int)frames[r].timestamps.count - 1, &report);
			frames[r].log.clear();
