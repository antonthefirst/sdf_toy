...
inline uint floatBitsToUint(float p) {
	uint ret = *((uint*)&p);
	return ret;
}
inline uvec2 floatBitsToUint(vec2 p) {
	uvec2 ret;
	ret.x = *((uint*)&p.x);
	ret.y = *((uint*)&p.y);
	return ret;
}
inline uvec3 floatBitsToUint(vec3 p) {
	uvec3 ret;
	ret.x = *((uint*)&p.x);
	ret.y = *((uint*)&p.y);
	ret.z = *((uint*)&p.z);
...
#if 0 // CUSTOM - already have types
/*
...
#if 0 // CUSTOM - already define these functions on these types
...
bits32 hashBits(float p);
bits32 hashBits(vec2  p);
bits32 hashBits(vec3  p);

/*
   Integers module.
*/

u32 u32Any(RNGSTATE_REF rng);                  // [0, INT32_MAX)
u32 u32To(RNGSTATE_REF rng, u32 max);          // [0, max)
u32 u32In(RNGSTATE_REF rng, u32 min, u32 max); // [min, max]
s32 s32In(RNGSTATE_REF rng, s32 min, s32 max); // [min, max]

/*
   Distributions module.
*/
float gaussian(RNGSTATE_REF rng, float mean, float st_dev);
u32  dice(RNGSTATE_REF rng, u32 sides);                        // Simulate an N-sided die, returns [0, SIDES)
u32  diceNoRepeat(RNGSTATE_REF rng, u32 sides, u32 prev_roll); // Same as dice(), but ensure that the roll is different from PREV_ROLL.
bool oneIn(RNGSTATE_REF rng, u32 chance);                      // 1:CHANCE that it returns true. (aka. dice(CHANCE) == 0)

/*
   Floats module.
*/
float floatUnit(RNGSTATE_REF rng);                      // [0.0, 1.0]
float floatEUnit(RNGSTATE_REF rng);                     // [0.0, 1.0)
float floatIn(RNGSTATE_REF rng, float min, float max);  // [min, max]
...
vec3 vec3InUnitSphere(RNGSTATE_REF rng); // #TODO
vec3 vec3OnUnitSphere(RNGSTATE_REF rng);
vec3 vec3OnUnitHemisphere(RNGSTATE_REF rng, vec3 normal); // #TODO
...
*/
INLINE u32 u32Any(RNGSTATE_REF rng) {
	return rngAdvance(rng);
}
INLINE u32 u32To(RNGSTATE_REF rng, u32 max) {
	/*
...
INLINE u32 u32In(RNGSTATE_REF rng, u32 range_min, u32 range_max) {
	return range_min + u32To(rng, range_max - range_min + u32(1)); // _to excludes the max, so +1
}
INLINE s32 s32In(RNGSTATE_REF rng, s32 range_min, s32 range_max) {
	return range_min + s32(u32To(rng, u32(range_max - range_min + 1))); // _to excludes the max, so +1
}
INLINE float gaussian(RNGSTATE_REF rng, float mean, float st_dev) {
	// box-muller transform
	float u1 = floatUnit(rng);
	float u2 = floatUnit(rng);
	float r = sqrt(-2.0f * log(u1));
	float theta = TAU*u2;
	return mean + st_dev*r*sin(theta);
}
INLINE u32 dice(RNGSTATE_REF rng, u32 sides) {
...
*/
INLINE bool oneIn(RNGSTATE_REF rng, u32 chance) {
...
INLINE float floatUnit(RNGSTATE_REF rng) {
	return unitFromBits(rngAdvance(rng));
}
INLINE float floatEUnit(RNGSTATE_REF rng) {
	return eunitFromBits(rngAdvance(rng));
}
/* #TODO make sure Snit and ESnit are actually unbiased down to the bit level. */
INLINE float floatSnit(RNGSTATE_REF rng) {
	u32 b = rngAdvance(rng);
	return unitFromBits(b) * sign((b & u32(0x8000000)) != u32(0) ? +1.0f : -1.0f);
}
INLINE float floatESnit(RNGSTATE_REF rng) {
	u32 b = rngAdvance(rng);
	return eunitFromBits(b) * sign((b & u32(0x8000000)) != u32(0) ? +1.0f : -1.0f);
}
INLINE float floatIn(RNGSTATE_REF rng, float range_min, float range_max) {
	return range_min + (range_max - range_min) * floatUnit(rng);
}

INLINE vec2 vec2In(RNGSTATE_REF rng, vec2 range_min, vec2 range_max) {
	vec2 ret;
	ret.x = floatIn(rng, range_min.x, range_max.x);
	ret.y = floatIn(rng, range_min.y, range_max.y);
	return ret;
}
INLINE vec2 vec2InUnitCircle(RNGSTATE_REF rng) {
	float a = floatEUnit(rng) * TAU;
	float r = floatSqrt(floatUnit(rng));
	return vec2(sin(a), cos(a)) * vec2(r);
}
INLINE vec2 vec2OnUnitCirle(RNGSTATE_REF rng) {
	float a = floatEUnit(rng) * TAU;
	return vec2(sin(a), cos(a));
}
INLINE vec3 vec3OnUnitSphere(RNGSTATE_REF rng) {
	/* #TODO source? */
	float s = floatUnit(rng) * TAU;
	float t = floatUnit(rng) * 2.0f - 1.0f;
	return vec3(floatSin(s), floatCos(s), t) * vec3(inversesqrt(1.0f + t * t));
}
INLINE vec3 vec3OnUnitHemisphere(RNGSTATE_REF rng, vec3 normal) {
	/* #TODO source? */
	vec3 v = vec3OnUnitSphere(rng);
	return v * sign(dot(v, normal));
}

INLINE quat quatAny(RNGSTATE_REF rng) {
...
INLINE uint hashBits(uint b) {
	return splitmix32(b);
}
INLINE uint hashBits(uvec2 b) {
	return hashBits(hashBits(b.x) ^ b.y);
}
INLINE uint hashBits(uvec3 b) {
	return hashBits(hashBits(hashBits(b.x) ^ b.y) ^ b.z);
}
INLINE bits32 hashBits(float p) { return hashBits(floatBitsToUint(p)); }
INLINE bits32 hashBits(vec2  p) { return hashBits(floatBitsToUint(p)); }
INLINE bits32 hashBits(vec3  p) { return hashBits(floatBitsToUint(p)); }
