...
bool ImGui_ImplGlfwGL3_CreateDeviceObjects()
{
    ...
    const GLchar *vertex_shader =
        ...
        "out vec2 Frag_UV;\n"
        "out vec4 Frag_Color;\n"
		"float D3DX_SRGB_to_FLOAT(float val) { if (val < 0.04045f) val /= 12.92f; else val = pow((val + 0.055f) / 1.055f, 2.4f); return val; }\n"
		"vec3 srgb2linear(vec3 srgb) { return vec3(D3DX_SRGB_to_FLOAT(srgb.x), D3DX_SRGB_to_FLOAT(srgb.y), D3DX_SRGB_to_FLOAT(srgb.z)); }\n"
        "void main()\n"
        "{\n"
        "	Frag_UV = UV;\n"
		"	Frag_Color = vec4(srgb2linear(Color.xyz),Color.w);\n"
    ...
    const GLchar* fragment_shader =
		...
		"void main()\n"
        "{\n"
		"   vec4 tex_col = texture( Texture, Frag_UV.st);\n"
        "	Out_Color = Frag_Color * tex_col;\n"
        "}\n";
...
static void ImGui_ImplGlfw_InstallCallbacks(GLFWwindow* window)
{
    ...
    //glfwSetScrollCallback(window, ImGui_ImplGlfw_ScrollCallback); // input_wrap steals this, but then calls this manually
