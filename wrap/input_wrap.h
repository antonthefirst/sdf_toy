...
inline int strToKey(const char* k) {
	if (!k[0] || !k[1]) //if it's a single character, not a key
		return k[0];
	if (strcmp(k, "left") == 0)
		return KEY_LEFT;
		...
		return KEY_END;
	else if (strcmp(k, "page up") == 0)
		return KEY_PAGE_UP;
	else if (strcmp(k, "page down") == 0)
		return KEY_PAGE_DOWN;
	else if (strcmp(k, "tab") == 0)
		return KEY_TAB;
	else if (strcmp(k, "caps lock") == 0)
		return KEY_CAPS_LOCK;
	else if (strcmp(k, "left shift") == 0)
		return KEY_LEFT_SHIFT;
	else if (strcmp(k, "right shift") == 0)
		return KEY_RIGHT_SHIFT;
	else if (strcmp(k, "left alt") == 0)
		return KEY_LEFT_ALT;
	else if (strcmp(k, "right alt") == 0)
		return KEY_RIGHT_ALT;
	else if (strcmp(k, "left ctrl") == 0)
		return KEY_LEFT_CONTROL;
	else if (strcmp(k, "right ctrl") == 0)
...
struct Input {
	struct Key {
		...
		inline bool getPress(const char* k) { return press[strToKey(k)]; }
		inline bool getRelease(const char* k) { return release[strToKey(k)]; }
		inline bool getDown(const char* k) { return down[strToKey(k)]; }
	} key;
	struct Mouse {
		float x, y;
		float dx, dy;
		...
		inline bool getPress(int b) { return press[b]; }
		inline bool getRelease(int b) { return release[b]; }
		inline bool getDown(int b) { return down[b]; }
		float wheel; //signed delta of wheel movement
	} mouse;
	struct Pad {
